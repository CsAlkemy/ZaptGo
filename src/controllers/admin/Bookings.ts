...
export class Bookings extends BaseController {
	constructor(private mongo: Mongo) {
		super(mongo);
		this.config.view = "admin/bookings";
	...
	async hotelBookings(@Res() res: Res, @Req() req: Req, @Session('user') session: Data) {
		if (req.method == METHOD.POST) {
			let action = new Action([
				new Button(ButtonType.SELF_LINK, makeUrl('/bookings/booking/{$1}', session), '<i class="fas fa-inbox" data-toggle="tooltip"  style="color: red" ></i>'),
				new Button(ButtonType.SELF_LINK, makeUrl('/bookings/booking/{$1}', session), '<i class="far fa-check-circle" style="color: green"></i>')]);
			...
			this.config.data["totalHotelSold"] = await this.mongo.BookingService.aggregate()
	...
	async bookings(@Res() res: Res, @Req() req: Req, @Session('user') session: Data) {
		if (req.method == METHOD.POST) {
			...
			this.config.data["totalPackageSold"] = await this.mongo.BookingService.aggregate()
